...
var skillsResult = _.sortBy(_.uniq(_.flattenDeep(skills)),_.toLower);
console.log('Task 1',skillsResult);



var people = _.map(data, function (object) {
    return {"name": object.name, "friends": object.friends}
});

var namesResult = _.map(_.sortBy(people, 'friends.length'), 'name');
console.log('Task 2',namesResult);

var friends = _.map(data, function (object) {
    return {"friends": _.map(object.friends, 'name')}
});

var friendsResult = _.uniq(_.flattenDeep(_.map(friends, 'friends')));
console.log('Task 3',friendsResult);
